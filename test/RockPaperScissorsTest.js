  ...
  beforeEach(function () {
    return RockPaperScissors.new({ from: owner })
      .then(async function (_instance) {
        ...
        return instance.generateNewGameKey();
      })
      .then(result => {
        playersHash = result;
      })
...
describe ("A suite of functions to test RockPaperScissors with new instance", () =>{ 
    ...
    beforeEach(function () {
      return instance.generateNewGameKey()
      .then(result => newHash = result)
      .catch(results => console.log(results)); 
    });

    beforeEach(function () {
      return instance.playerOneStartGameWithFunds(newHash, 0, 0, { from: playerOne, value: web3.toWei('0.1', 'ether') })
      .catch(results => console.log(results)); 
    });
    ...
    it("should allow Player one to win.", async function () {
      ...
      await instance.playerTwoEnterAndPlayWithFunds(newHash, 1, 0, { from: playerTwo, value: web3.toWei('0.01', 'ether') })

      return instance.withdrawBalance(newHash, { from: playerTwo })
        .then(result => {
          assert.isTrue(true);
        })
        .catch(result => {
    ...
    it("should allow a draw.", async function () {
      ...
      return instance.playerTwoEnterAndPlayWithFunds(newHash, 0, 0,{ from: playerTwo, value: web3.toWei('0.01', 'ether') })
        .then(result => {
          assert.isTrue(false);
        })
        .catch(result => {
